...
| 8 | Control estructural documentado | Sección nueva en `docs/security.md`: el donante no designa consignatario ni rastrea sus bienes a destino, y la tarima mixta es un control anti-abuso deliberado. Se documenta porque no parecen controles al leer el código —parecen decisiones de producto— y alguien podría "arreglarlas" sin saber qué desarma. Referencia cruzada en el spec de la Fase 18. | 🟢 Baja | ✅ Done |
| 9 | `CLAUDE.md` + roadmap | Política de prevención de riesgos en las reglas de negocio (los 5 controles y el alcance declarado: registro y escalamiento, sin screening ni verificación documental), `RiskReview` en el modelo de dominio y Revisiones en el sidebar. Totales actualizados. | 🟢 Baja | ✅ Done |
