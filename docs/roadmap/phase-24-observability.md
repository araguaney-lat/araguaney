...
| 4 | Verificar Sentry de punta a punta | Verificado el 3/8/2026 contra producción: un error de navegador llegó como issue con entorno, release y traza resuelta a código fuente, con bloqueador activo. El SDK inicializado no prueba nada, y esta verificación lo demostró: encontró el SDK del cliente sin cargar, la organización mal escrita y los eventos frenados por bloqueadores. El entorno `preview` no se probó, por decisión. | 🟢 Baja | ✅ Done |
| 4b | Blindar la llave pública de Sentry | El DSN del frontend es público por diseño y seguirá siéndolo: el SDK lo necesita. Lo acotable quedó acotado — purgado de IP activo, y límite en el borde de Vercel sobre `/monitoring` (200/min por IP, hoy en registro), que es por donde pasa todo el tráfico desde que hay túnel. **Vercel no factura lo que su firewall bloquea**, así que ahí se cierra el riesgo de EDoS. Los dominios permitidos quedan en `*` a propósito: se midió con eventos sintéticos y, con el túnel, aceptaba hasta un origen ajeno, porque el evento ya no sale del navegador. El límite por llave es de pago. | 🟢 Baja | ✅ Done |
...
| 6 | Alerta de correo rebotado en volumen | `bounce_watchdog_cron` mide dos señales distintas: volumen total en la ventana (algo se rompió de nuestro lado) y concentración en un dominio (un proveedor nos filtra, aunque el total se vea sano). El aviso nombra **dominios, nunca direcciones**: la señal se lee igual y ninguna dirección sale de la base hacia un chat. Umbrales en el entorno, encendidos por defecto — apagados reproducirían el fallo silencioso que ataca esta fase. | 🟠 Media | ✅ Done |
| 7 | Presupuesto de ruido | `alert_budget.should_send` agrupa por **identidad del problema** (ruta + excepción, cron + excepción, lista de crons rezagados), no por texto ni severidad: un problema distinto siempre suena. Reserva la ventana con `SET NX`, atómico, porque dos workers que fallan a la vez deben mandar una alerta y no dos. Falla abierto: sin Redis se manda todo, que un canal ruidoso se arregla leyéndolo y uno mudo no se nota. | 🟢 Baja | ✅ Done |
| 8 | Uptime externo | Monitor gratuito contra `/health/jobs` cada 5 min, alertando ante cualquier código distinto de 200. Si Railway se cae entero, ninguna alerta interna va a salir — por definición. Dos trampas resueltas en el camino: el plan gratuito comprueba con `HEAD` y no deja elegir método, así que hubo que permitirlo en la regla de Cloudflare (`TRACE` sigue bloqueado) y registrar la ruta para `GET` y `HEAD`. Runbook en [`docs/observability.md`](../observability.md). | 🟢 Baja | ✅ Done |
| 9 | Documentar qué se vigila | [`docs/observability.md`](../observability.md): qué dispara cada alerta, a qué canal llega, qué se espera de quien la recibe, y los huecos declarados con nombre y consecuencia. Incluye cómo agregar una alerta nueva sin romper la política. Un hueco documentado se puede planear; uno implícito no. | 🟢 Baja | ✅ Done |
| 10 | Roadmap + `CLAUDE.md` | La política queda en la sección 10 de `CLAUDE.md`, con los cuatro pasos para agregar trabajo de fondo (decorar, declarar ventana de latido, agrupar con `budget_key`, escribir lo que quede sin cubrir) y los dos límites que acepta a propósito: la observabilidad nunca tumba una petición y falla abierta donde puede. La Definition of Done gana su renglón. | 🟢 Baja | ✅ Done |
