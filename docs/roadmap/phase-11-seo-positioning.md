...
| 11 | `hreflang` ES/EN | Tags `hreflang` para señalar versiones de idioma a buscadores. | 🟢 | ✅ Done — desbloqueada migrando a **locale por URL** (ES sin prefijo, EN con `/en/...`, slugs traducidos vía `routes.ts`). Implementada en 6 sub-proyectos: **1** infra (`app/[lang]`, middleware `handleI18n`, `getLocale` por header) + home + `/centro-de-acopio`; **2a** pilares + legales + contacto; **2b** guías + glosario; **2c** `/necesidades` + 7 categorías; **2d** fichas QR bilingües (por `Accept-Language`, sin URL-locale — datos ES) + `eventos/[slug]` documentado ES-only; **3** panel/studio con prefijo `/en/dashboard`·`/en/studio` (mismo slug + cookie-sync, auth-gated → sin hreflang). ~27 páginas públicas ES/EN con `hreflang`/canonical/x-default vía `alternates`. |
...
| # | Tarea | Descripción | Prioridad | Estado |
|---|-------|-------------|-----------|--------|
| 25 | Breadcrumbs visibles + OG cards por página + Organization + landings por categoría | Cerrar los quick-wins de posicionamiento restantes. | 🟡 | ✅ Done — (a) breadcrumbs visibles (`src/components/Breadcrumbs.tsx`) en las 6 guías, el hub `/guias`, el glosario, los 3 pilares y las landings de categoría, con la misma data que el `BreadcrumbList` JSON-LD (una sola fuente `CRUMBS`); (b) OG cards por página: endpoint dinámico `app/og/route.tsx` (título + eyebrow, sanitizado y con cap de longitud) + helper `ogImageUrl()`, aplicado a guías/glosario/pilares/hub/categorías (antes usaban el logo `DEFAULT_OG_IMAGE`); (c) `Organization` schema enriquecido con `knowsAbout`, `areaServed` y `contactPoint` (E-E-A-T); (d) landings por categoría `/necesidades/[category]` (7: medicamentos, insumos-medicos, alimentos, agua, higiene, herramientas, equipo-de-rescate) con copy editorial propio (qué se acepta / qué no) + total en vivo, `BreadcrumbList`, en sitemap y `llms.txt`, enlazadas desde `/necesidades` (internal linking). Datos en `src/lib/needs-categories.ts`. Patrón ISR sin `generateStaticParams` (evita el hang de build en Vercel, como `eventos/[slug]`). Build y `tsc` verdes; breadcrumbs, OG endpoint, categorías (200 + 404 en slug inválido), Organization y sitemap verificados contra el server de producción local. |
