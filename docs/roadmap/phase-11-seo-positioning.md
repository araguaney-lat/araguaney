...
| 4 | Optimizar `/necesidades` como SEO asset | Meta title/description dinámicos por campaña y categoría. Contenido indexable (SSR/ISR, no solo cliente). Mantener edge-cache. Mapea al Cluster D ("qué falta"). | 🟠 | 🟡 Parcial — metadata + canonical ya resueltos en Grupo B (SSR, edge-cache con `revalidate=300`). Dinámico por campaña/categoría sigue bloqueado: no hay endpoint público de listado de campañas (mismo bloqueo que sitemap, tarea 8). |
