...
| # | Tarea | Descripción | Prioridad | Estado |
|---|-------|-------------|-----------|--------|
| 5 | Enriquecer `Organization` con `sameAs` + `foundingDate` + `founder` | Anclar la entidad a perfiles autoritativos (LinkedIn, GitHub, X/Instagram, Crunchbase). Señal de entidad = mayor correlación con citas de IA que los backlinks. | 🟠 | 🟡 En progreso — `foundingDate: "2026"` + `founder` (Person: Antony E Delgado Casanova) ya en `ORGANIZATION_SCHEMA`. `sameAs` cableado a `BRAND_SAME_AS` en `seo.ts` (fuente única de verdad, solo se emite si no está vacío) pero **vacío**: bloqueado hasta crear las redes oficiales. Al crearlas, agregar cada URL ahí (una línea) — también sirve para el QID de Wikidata (task 6). |
