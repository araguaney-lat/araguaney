...
| 7 | Página `/nosotros` (entity home / about) | Misión, autoría/equipo, estándares que respaldamos, `sameAs`. Refuerza E-E-A-T y da un ancla de entidad indexable. Con `AboutPage`/`Organization` schema. | 🟡 | ✅ Done — página bilingüe `/nosotros` (EN `/about`): hero/misión, por qué existe, cómo lo hacemos (4 pasos), estándares (WHO/IFRC/IOM/UNSPSC/GS1), privacidad por diseño, founder (una línea: fundado en 2026 por Antony E Delgado Casanova), CTA + cross-links. Emite `ORGANIZATION_SCHEMA` (con `sameAs`/founder/foundingDate — entity home) + nuevo `aboutPageSchema` + `BreadcrumbList`. Registrada en `routes.ts` + `middleware` matcher + `sitemap.ts` + `llms.txt`/`llms-full.txt` + **enlace en el footer** (`HomeFooter`). Verificado corriendo el build: ES/EN 200, schema con founder+sameAs, canonical www, hreflang es/en/x-default. Destino para las bylines de la task 9. Spec: `docs/superpowers/specs/2026-07-24-nosotros-entity-home-design.md`. |
...
| 9 | Autoría E-E-A-T en guías | `author` + enlace a `/nosotros` en las 6 guías (bylines). Señal de experiencia/autoridad para rich results y citas. | 🟢 | ✅ Done — byline visible en las 6 guías junto a la fecha: "Por el equipo de Araguaney" (ES) / "By the Araguaney team" (EN), enlazado a `/nosotros` (helper `authorByline` en `content-dates.ts`). El `author` del `Article` schema sigue siendo la Organization (autoría institucional, honesto). Verificado corriendo el build: byline presente en las 6, enlaza a `/nosotros` (ES) / `/en/about` (EN). |
