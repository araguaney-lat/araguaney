...
| 8 | Fotos en R2 | upload-url → confirm desde el enlace de gestión (patrón `messaging`): solo JPEG/PNG/WebP, 5 MB, máx 5 por donación, y solo mientras la donación siga en `REGISTERED`. La llave la arma el servidor con el id de la donación — el nombre del archivo del cliente nunca la toca — y confirmar solo vale dentro de esa carpeta. Lectura con URL firmada de vida corta, para el donante y para el centro que va a recibir; la ficha pública del QR **no** las expone. Se borran del almacenamiento al quitarlas, al cancelar y al vencer. | 🟠 Media | ✅ Done |
...
| 13 | Confirmación + gestión | Página de "revisa tu correo", `/donacion/[token]` (editar renglones, fotos y cancelar en `REGISTERED`; solo lectura después) y reenvío del enlace. **La página de gestión faltaba**: se dio por hecha, pero el enlace del correo de confirmación respondía 404 en producción hasta la task 8. | 🟠 Media | ✅ Done |
...
| 19 | Seguridad de cierre | Pasada final sobre los endpoints de la fase. Tres huecos cerrados: (a) el alta delataba si un correo ajeno ya tenía una donación abierta — ahora responde igual en ambos casos y, si es el dueño, se le reenvía su confirmación; (b) los textos del formulario público no tenían tope de longitud ni de renglones (alta, edición por el donante y recepción) — ahora sí, en el esquema; (c) lo autenticado declara `no-store`, para que un cambio de capa de borde no deje datos de un centro en una caché compartida. Verificado además: límite de tasa en toda ruta nueva, ficha pública sin PII, 404 uniforme anti-enumeración y aislamiento por centro con tests desde la posición del atacante. | 🟠 Media | ✅ Done |
| 20 | Legal | Aviso de privacidad ES/EN: categoría nueva "personas que registran su donación en línea" (qué se recaba, para qué, qué es obligatorio), finalidad del pre-registro y aclaración de que la ficha del QR nunca muestra datos personales. Plazos de conservación ya declarados en la task 11. `CLAUDE.md`: el NO-objetivo #4 se reescribe (la donación anónima sigue siendo el default, pero el donante puede identificarse), se añaden `/donar` y `/d/[code]` a las rutas públicas, `Donor` y `Donation` al modelo de dominio, y se ajusta la Definition of Done. | 🟠 Media | ✅ Done |
...
| 21 | Email "salió en envío" | Al despachar (`CLOSED → SHIPPED`), se avisa a quien pre-registró la carga que va en ese envío: donación → intake → cajas → tarima → envío, solo quienes dejaron correo y un aviso por donante. Encolado después del commit — un fallo de correo no puede deshacer un despacho. De paso se conecta el correo de recepción, cuya plantilla existía desde la task 10 pero que nadie disparaba. | 🟠 Media | ✅ Done |
