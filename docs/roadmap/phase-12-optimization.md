...
| 12 | Covering index (`INCLUDE`) para agregación | Avanzado: habilitar index-only scans en el panel nacional si se vuelve lento con volumen real. Medir primero. | 🟢 | ⬜ Bloqueada — la tarea explícitamente pide medir antes de actuar ("si se vuelve lento con volumen real"), y la metodología de esta fase es la misma: "Sin medición no hay optimización — solo suposiciones". No hay baseline de volumen real todavía (depende de Grupo F, load testing, aún pendiente). No implementar a ciegas. |
