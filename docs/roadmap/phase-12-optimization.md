...
| 12 | Covering index (`INCLUDE`) para agregación | Avanzado: habilitar index-only scans en el panel nacional si se vuelve lento con volumen real. Medir primero. | 🟢 | ✅ Done — implementada como trabajo preventivo (decisión del usuario, no por necesidad medida — a 6000 cajas/20 centros el panel nacional no está lento, ver tarea 26). Migración `021`: reemplaza `ix_boxes_sealed_center` (Grupo A, tarea 5) por `ix_boxes_sealed_center_covering` — mismo índice parcial `(center_id) WHERE status='SEALED'` más `INCLUDE (quantity, weight_kg)`, las dos columnas que `stock_by_center`/`weight_by_center`/`summary_totals` necesitan. `ALTER INDEX ... ADD INCLUDE` no existe en Postgres — la migración hace `CREATE INDEX CONCURRENTLY` (nuevo) + `DROP INDEX CONCURRENTLY` (viejo), sin bloquear escrituras. Verificado en Postgres real: tras un `VACUUM` (fija el visibility map — sin esto Postgres no puede omitir el heap aunque el índice cubra las columnas), la consulta de un centro usa `Index Only Scan ... Heap Fetches: 0` cuando es la única opción viable. A volumen actual (6000 cajas) el planner sigue prefiriendo otros índices/`Seq Scan` para casos de baja selectividad (mismo hallazgo que tarea 26) — el índice está listo y probado para cuando el volumen lo justifique, no cambia el comportamiento observable hoy. |
