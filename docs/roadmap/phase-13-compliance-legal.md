...
| 7 | Versionado de documentos legales | Cada documento legal lleva versión + fecha de última actualización visible. Cuando cambie el aviso/términos, incrementar versión; usuarios existentes re-aceptan en próximo login si `accepted_terms_version` quedó atrás. | 🟡 | 🟡 Parcial — cada documento lleva `version` + fecha visible (v1.0, 2026-07-02). La re-aceptación forzada en login depende de la columna `accepted_terms_version` de la task 3. |
