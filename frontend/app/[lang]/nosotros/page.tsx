...
const CONTENT: Record<Locale, Content> = {
  es: {
    ...
    privacyP:
      "Araguaney no registra ni un solo dato de las personas beneficiarias: gestiona inventario, trazable de la caja al envío. De quien dona depende de la persona. En el centro, la donación es anónima por default: registrar sus datos es una casilla opcional que alguien tiene que marcar a propósito. Registrar la donación en línea sí pide un correo, porque el código QR y el enlace para modificarla o cancelarla llegan por ahí. En ambos casos esos datos tienen plazo declarado y se purgan solos: lo que nadie confirma se borra. Menos superficie de datos sensibles, menos riesgo, y una herramienta concentrada en lo que importa: que la ayuda llegue.",
  ...
  en: {
    ...
    privacyP:
      "Araguaney stores no data whatsoever about aid recipients: it manages inventory, traceable from box to shipment. For donors, it is up to the person. At the center the donation is anonymous by default: recording their details is an optional checkbox someone has to tick on purpose. Registering a donation online does ask for an email, because the QR code and the link to change or cancel it are sent there. In both cases that data has a declared retention period and is purged automatically: whatever nobody confirms is deleted. Less sensitive-data surface, less risk, and a tool focused on what matters: getting aid delivered.",
